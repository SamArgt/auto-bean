...
3. If the request is import-derived transaction posting, inspect the reviewed `statements/parsed/*.json` inputs and any bounded `.auto-bean/memory/import_sources/` hints before invoking the write workflow.
4. Inspect the affected workspace files and summarize:
   - what changed
   - why it changed
   - which files are affected
5. Draft the scoped structural mutation directly in the working tree.
   - For Beancount transaction mutations, invoke `$auto-bean-write` and treat its drafted entries, validation result, duplicate/transfer/currency risks, and clarification state as inputs to this apply workflow.
...
6. If the mutation writes Beancount transactions derived from reviewed import evidence, run apply-level reconciliation checks on the drafted workspace result before any finalization claim.
...
7. If the mutation writes Beancount transactions derived from reviewed import evidence and the drafted result still depends on an ambiguous or unfamiliar interpretation, stop at a clarification checkpoint before any finalization claim.
...
8. When the user answers a clarification request, resume from the same clarification record before any finalization claim.
   - Use `$auto-bean-write-transactions` if the answer requires changing drafted transaction entries.
...
9. If the mutation writes Beancount transactions derived from reviewed import evidence, update the relevant `statements/import-status.yml` entries after those transactions are written in the workspace.
...
10. Confirm post-mutation validation before any finalization claim.
...
11. Present a concise post-mutation review package before any commit or push step:
...
12. Ask the user for a decision on each reconciliation finding before finalization.
...
13. Apply the user-selected action for each finding in the working tree.
...
14. Ask for explicit approval before commit or push finalization only after the finding decisions have been applied and the current workspace result has been revalidated. If approval is denied, deferred, or cannot be obtained, leave the mutation in the working tree and describe it as unfinalized.
15. After validation succeeds and the workflow is explicitly finalized, update the same `statements/import-status.yml` entries to `status: done`.
16. Suggest source-context create or update only after a trustworthy finalized outcome, including reusable clarification outcomes when they clearly fit the source-specific memory boundary; let the orchestrator decide whether to write it.
17. When the user asks how to undo a committed mutation, explain the git-backed recovery path from the recorded commit history.
