...
10. If approval is denied, deferred, or blocked, leave the change unfinalized and record blocked or rejected outcomes under `.auto-bean/artifacts/` when durable troubleshooting context will help.
11. Create a proposal artifact under `.auto-bean/proposals/` only when the user asks for deeper review or when the change is risky enough to justify durable supplemental context.
12. If approval is granted, commit or push as approved.
13. Distinguish clearly between "the working tree changed" and "the change was accepted into history."
