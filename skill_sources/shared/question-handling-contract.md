...
When a sub-agent stage is invoked by `$auto-bean-import`, the sub-agent NEVER ask the user directly. It records safe progress, persists the question in its stage-owned artifact, and returns only question ids, artifact path, and operational blocker flags to `$auto-bean-import`.
